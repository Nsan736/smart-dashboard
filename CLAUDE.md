...
- 地震情報(`QuakeStore`): P2P地震情報 JSON API v2 の /history?codes=551&limit=10 を使う(転送時約3KB。気象庁の list.json は約20KBで件数を絞れない)。レート制限は60リクエスト/分で、自動更新は最短5分
  ...
  - 観測点ごとの震度を地図に塗り分ける案(未実装): 気象庁の震度観測点の一覧 https://www.data.jma.go.jp/eqev/data/intens-st/stations.json (4,368点、名前・緯度経度、転送時約70KB、必要な項目だけにすると約160KB/gzipで約54KB)を同梱すれば、P2P地震情報の観測点名と約99%が名前で一致する(2026-09-20に確認)
