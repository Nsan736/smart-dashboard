...
- 気圧の変化: 実測(CMAltimeter)と予報(Open-Meteo の hourly surface_pressure。天気のリクエストに past_hours=24 と一緒に足してあり、リクエストは1回のまま)を1本の線でつなぐ(`PressureSeries`)
  ...
  - 詳細画面(`PressureDetailView`): 天気タブの気圧の節と、ホームの気圧カードから開く。横スクロールで過去4日〜16日先。表示の幅は 1日/1週間/全体(初期値1週間)。開いたときは「今」が左から25%の位置で、右にスクロールすると先の予報が見られる。表示する点は幅に応じて間引く(全体=1時間ごと、1週間=30分ごと、1日=間引かない)。日付の区切り(土曜=青、日曜・祝日=赤)、5hPaごとの横線、1013hPaの線、急な低下(3時間で設定値以上)の時間帯の色分け、未来は点線。下に表示中の期間の最高・最低・平均と、1日ごとの最高・最低
    - 値の表示はタップだけ(`chartGesture` の SpatialTapGesture で `selectXValue`)。長押しでなぞる標準の操作は置き換えてあり、指を動かしたときは常に横スクロールになる。別の場所をタップすると切り替わり、グラフの外(値の行や下の一覧)をタップするか、幅を切り替えると消える
