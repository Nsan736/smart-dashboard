...
- 気圧の変化: 実測(CMAltimeter)と予報(Open-Meteo の hourly surface_pressure。天気のリクエストに past_hours=24 と一緒に足してあり、リクエストは1回のまま)を1本の線でつなぐ(`PressureSeries`)
  - 実測は `PressureRecorder` がアプリの起動中に5分に1回記録し、100日分を Application Support に保存する(キャッシュ削除では消さない)。1日(UTC)ごとのファイルに分け、記録のたびに書くのは当日の分だけ(`PressureSampleCodec`、1件約25バイト)
  - 予報モデルの履歴(`PressureHistoryStore`、過去100日〜7日先、1時間ごと、1地点の分だけ)も端末に保存する。100日より古いものは削除する
    - 過去の分のまとめての取得は Open-Meteo の Historical Forecast API(historical-forecast-api.open-meteo.com、hourly=surface_pressure だけ、約10KB)。Forecast API の past_days=92 は古い約19日分が null で返る(2026-09-20に確認。モデルを変えても同じ)ので使わない。自動はWi-Fiなど従量制でない回線のときだけ、モバイル通信では詳細画面のボタンで手動
    - 以後は、天気の更新(past_hours=24 を含む)のたびに追記する。通信は増えない。空白が24時間を超えた分と7日先までの予報は、詳細画面を開いたときに、足りない分だけ(past_hours=24〜168、forecast_hours=168、約1KB、最短3時間間隔)を取る。空白が7日を超えたら、まとめて取り直す
    - 50km以上離れた地点に変わったら、保存した分を捨てて、その地点で取り直す(気圧は標高で大きく変わり、別の地点の値とはつなげないため。実測の記録は消さない)
    - timeformat は unixtime のまま。gzip後の転送量はISO形式のほうが約6%小さいが(2026-09-20に計測)、差は1回約0.5KBで、時刻の解釈を天気と同じにしておくほうを選んだ
  - 詳細画面(`PressureDetailView`): 天気タブの気圧の節と、ホームの気圧カードから開く。横スクロール(過去約100日〜7日先)、表示の幅は1日/1週間/1か月(初期値1週間)、開いたときは「今」が左から75%の位置。表示する点は幅に応じて間引く(1か月=1時間ごと、1週間=30分ごと、1日=表示中の前後1.5日だけ5分ごとで、ほかは1時間ごと)。日付の区切り(土曜=青、日曜・祝日=赤)、5hPaごとの横線、1013hPaの線、急な低下(3時間で設定値以上)の時間帯の色分け、未来は点線。長押しでなぞると日時・気圧・実測か予報か・前3時間の変化を表示する。下に表示中の期間の最高・最低・平均と、1日ごとの最高・最低
