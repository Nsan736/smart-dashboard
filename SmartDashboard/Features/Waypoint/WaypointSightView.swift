...
enum WaypointSightMode: String {
    case camera
    case compass
}

/// カメラ越しの表示と、コンパス表示。開いている間だけ、カメラ・GPS・モーションを動かす。
struct WaypointSightView: View {
    @Environment(AppEnvironment.self) private var env
    @Environment(\.scenePhase) private var scenePhase
    @State private var mode: WaypointSightMode
    /// 表示する地点。nil なら全部。
    ...
    var body: some View {
        ...
        .navigationTitle(mode == .camera ? "カメラで見る" : "コンパス")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ToolbarItemGroup(placement: .topBarTrailing) {
                Menu {
                    Picker("表示する地点", selection: $selectedID) {
        ...
        .onAppear {
            isVisible = true
            tracker.onLocation = { [store = env.waypoints] in store.setOrigin(latitude: $0.latitude, longitude: $0.longitude) }
            startAll()
        }
        .onDisappear {
            isVisible = false
    ...
    private var currentTargets: [WaypointTarget] {
        guard let coordinate = tracker.coordinate else { return [] }
        let list = env.waypoints.waypoints.filter { selectedID == nil || $0.id == selectedID }
        return WaypointTarget.make(list, latitude: coordinate.latitude, longitude: coordinate.longitude)
    }
    ...
    private func cameraLayer(_ targets: [WaypointTarget]) -> some View {
        ...
        ScrollView {
            VStack(spacing: 12) {
                ...
                if let heading = tracker.compassHeading {
                    ...
                    WaypointCompassDial(heading: heading, targets: targets)
                        .aspectRatio(1, contentMode: .fit)
                        .frame(maxWidth: 360)
                        .padding(.horizontal)
                    ForEach(targets.reversed()) { target in
                        compassRow(target, heading: heading)
                    }
                    if targets.isEmpty {
                        Text(tracker.coordinate == nil ? "現在地を取得中…" : "表示する地点がありません").foregroundStyle(.secondary)
                    }
                } else {
    ...
    private func compassRow(_ target: WaypointTarget, heading: Double) -> some View {
        let turn = WaypointMath.difference(from: heading, to: target.bearing)
        return HStack(spacing: 10) {
            Circle().fill(WaypointPalette.color(target.waypoint.colorIndex)).frame(width: 14, height: 14)
            VStack(alignment: .leading, spacing: 0) {
                Text(target.waypoint.name).font(.headline).fixedSize(horizontal: false, vertical: true)
...
/// カメラの映像の上に、画面内にある地点の印を描く。1枚の Canvas にまとめ、遠い地点から描く(近い地点が手前)。
/// 画面の外にある地点は何も描かない。
struct WaypointOverlay: View {
    let targets: [WaypointTarget]
    ...
    static func drawMark(_ context: inout GraphicsContext, at point: CGPoint, target: WaypointTarget) {
        ...
        context.fill(Path(ellipseIn: innerRect), with: .color(WaypointPalette.color(target.waypoint.colorIndex)))
        ...
        // 名前と距離(薄い背景を付ける)
        let label = context.resolve(Text("\(target.waypoint.name)  \(WaypointMath.distanceText(target.distance))")
...
/// 方位の目盛りと、地点の方向を示す矢印。端末の上端の向きが、常に上になる。
struct WaypointCompassDial: View {
    let heading: Double
    let targets: [WaypointTarget]
    ...
    var body: some View {
        Canvas(rendersAsynchronously: false) { context, size in
            ...
            // 地点の方向の矢印(遠い地点から描く)
            for target in targets {
                let color = WaypointPalette.color(target.waypoint.colorIndex)
