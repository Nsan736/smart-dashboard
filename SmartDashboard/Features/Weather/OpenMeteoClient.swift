...
struct OpenMeteoClient: WeatherAPI {
    ...
    /// 使う変数だけを指定し、期間も必要な分(15分値は2時間、1時間値は今後24時間と、気圧のグラフ用に過去24時間、日別は7日)に絞る。リクエストは1回。
    static func makeURL(latitude: Double, longitude: Double) -> URL {
        ...
        c.queryItems = [
            ...
            URLQueryItem(name: "hourly", value: "temperature_2m,precipitation_probability,weather_code,surface_pressure"),
